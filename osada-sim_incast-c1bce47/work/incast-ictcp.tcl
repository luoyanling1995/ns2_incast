...
# NoDelayedACK (for ICTCP)
Agent/TCPSink/ICTCP set interval_ 0ms
Agent/TCPSink/ICTCP set wnd_ $adv_win
...
for {set i 0} {$i < $svr_num} {incr i} {
    ...
    set snk_($i) [new Agent/TCPSink/ICTCP]
...
proc check_trans {} {
	...
	global bytes_prev link_del_us tick
	set time 0.0001
	set now [$ns now]
	...
	# check traffic and inform total taffic to each TCP sink agent
	# puts "$now: Server: 0, bytes: [$snk_(0) set bytes_]"
	set total_bytes 0
    set weighted_total_bytes 0
	for {set i 0} {$i < $svr_num} {incr i} {
		set total_bytes [expr $total_bytes + [$snk_($i) set bytes_]]
		set weighted_total_bytes [expr $weighted_total_bytes \
		  + [$tcp_($i) set rtt_] * ([$snk_($i) set bytes_] - $bytes_prev($i))]
		set $bytes_prev($i) [$snk_($i) set bytes_]
	}
	...
	# For ICTCP: Finding SubSlotT
	if {$total_bytes != 0} {
		set sub_slot_T [expr $weighted_total_bytes / $total_bytes]
	} else {
		# puts "$now: subslot = 100us"
		set sub_slot_T [expr $link_del_us * 4 * 10]
	}

	for {set i 0} {$i < $svr_num} {incr i} {
		$snk_($i) set total_bytes_ $total_bytes
		# exchange RTT value (not smooothed) to the Client from Servers
		# puts -nonewline "$now: server0 rtt: [$tcp_(0) set rtt_]tick "
		# puts "srtt: [$tcp_(0) set srtt_] / 8 tick"
		$snk_($i) set reverse_rtt_ [$tcp_($i) set rtt_]
		$snk_($i) set sub_slot_T_ $sub_slot_T
	}

    update_link_del

	$ns at [expr $now + $sub_slot_T * $tick] "check_trans"
}
